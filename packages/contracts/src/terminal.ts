...
export const TerminalWriteInput = Schema.Struct({
   ...
   *
   * Browser xterm classifies physical keys as `terminal` and capability replies
   * generated by its emulator as `renderer`. TUI keyboard input is decoded by
   * the outer terminal first, which can accidentally surface that outer
   * terminal's replies as key sequences. The server always filters reply-shaped
   * bytes from `renderer` and `keyboard` writes so they cannot be injected into
   * an embedded pager or shell.
   */
  inputSource: Schema.optional(Schema.Literals(["terminal", "keyboard", "renderer", "paste"])),
});
