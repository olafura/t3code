...
/**
 * Resolves a terminal path link against the session cwd: "~/" expands from the
 * cwd's user home, relative paths join onto the cwd. Position suffixes are the
 * caller's job (client-runtime's splitFilePathPosition), keeping this module
 * dependency-free.
 */
export function resolveTerminalPath(path: string, cwd: string): string {
  if (path.startsWith("~/")) {
