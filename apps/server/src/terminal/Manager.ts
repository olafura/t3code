...
export const TERMINAL_SEQUENCE_GRAMMAR: ReadonlyArray<TerminalSequenceDescriptor> = [
  ...
  {
    name: "XTVERSION query",
    kind: "csi",
    query: { body: ">[0-9;]*q", samples: [">q"] },
  },
  {
    name: "Kitty keyboard protocol query/report",
    kind: "csi",
    response: {
      body: "\\?[0-9;]+u",
      stripFromOutput: true,
...
function createTerminalSpawnEnv(
  ...
  // The PTY always feeds a headless xterm.js emulator, so TERM must describe
  // that emulator — not whatever terminal (if any) launched the server daemon.
      ...
      const action: DrainProcessEventAction = yield* Effect.sync(() => {
        ...
        if (nextEvent.type === "output") {
          ...
          if (sanitized.historyText.length > 0) {
            session.history.append(sanitized.historyText);
          }
    ...
    if (
      ...
      const refreshed = yield* Effect.exit(
        acquireSubprocessInspector.pipe(
          Effect.flatMap(({ inspector: currentSubprocessInspector }) =>
            currentSubprocessInspector(process.pid),
          ),
        ),
      );
