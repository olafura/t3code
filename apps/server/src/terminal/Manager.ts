...
function shouldStripCsiSequence(body: string, finalByte: string, responsesOnly = false): boolean {
  ...
  // DECRQM mode query. Keep it in the live stream so the client can answer,
  // but never replay it from persisted history.
  ...
  // Kitty keyboard protocol query/reply (?u). Keep the bare query live while
  // stripping replies with flag parameters; restore-cursor (bare u) stays.
  if (finalByte === "u" && body.startsWith("?")) {
    return responsesOnly ? /^\?[0-9;]+$/.test(body) : true;
  }
  return false;
}
