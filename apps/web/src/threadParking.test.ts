...
describe("thread parking", () => {
  ...
  it.each([true, false])("checks fresh parking state after success: %s", async (isParked) => {
    const s = setup();
    const completion = deferredSuccess();
    let canNavigate = !isParked;
    const pending = s.parking.run("a", () => completion.promise, undefined, () => canNavigate);
    canNavigate = isParked;
    completion.resolve(AsyncResult.success(undefined));
    await expect(pending).resolves.toEqual({ status: "success" });
    expect(s.navigate).toHaveBeenCalledTimes(isParked ? 1 : 0);
    expect(s.fallback).not.toHaveBeenCalled();
